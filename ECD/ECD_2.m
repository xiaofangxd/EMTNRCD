function [Mod, chromosome, Division, EP, time] = ECD_2(adjacent_array, maxgen, pop_size, ...
    p_mutation, p_migration, p_mu_mi, num_neighbor, pre_Result, Threshold, timestep_num)
...
adjacent_array(adjacent_array~=0) = 1;
...
%     disp(['time_stamp=; ', num2str(timestep_num), ' gen=: ', num2str(t)]);
%     pltf = zeros(length(EP),2);
%     for k = 1:length(EP)
%         pltf(k,:) = EP(k).fitness_1;
%     end
%     [~, b] = sort(pltf(:,1));
%     plot(pltf(b,1), pltf(b,2));
%     drawnow;
end
