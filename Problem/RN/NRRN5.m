...
% No.   Name                              Samples Features Classes
    ...
    methods
        ...
        function obj = NRRN5()
            % Load data
            dataNo = obj.Global.ParameterSet(5);
            str = {'karate','polbooks','football','lesmis','dolphins','celegansneural'};
            CallStack = dbstack('-completenames');
            load(fullfile(fileparts(CallStack(1).file),'Dataset_RNA_1.mat'),'DatasetA');
%             Data = Dataset.(str{dataNo});
            obj.TrainIn     = DatasetA.(str{dataNo});
            load(fullfile(fileparts(CallStack(1).file),'Dataset_RNY_1.mat'),'Datasety');
