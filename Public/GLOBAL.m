...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function notermination = NotTermination(obj,Population, nmi)
...
%                     drawnow();
                    ...
                    [~,~]  = mkdir(folder);
                    runtime = obj.runtime;
                    save(fullfile(folder,sprintf('%s_%s_M%d_D%d_%d.mat',func2str(obj.algorithm),class(obj.problem),obj.M,obj.D,obj.run)),'Population','nmi','runtime');
                    % Display the results
...
%                     Draw(Population.objs);
                    ...
                    save(fullfile(folder,sprintf('%s_%s_M%d_D%d_%d.mat',func2str(obj.algorithm),class(obj.problem),obj.M,obj.D,obj.run)),'result','metric','nmi');
                end
            end
        end
