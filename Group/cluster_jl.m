...
SumTot = sum(M); % Sum of weight incident to nodes in C
SumIn = diag(M); % Sum of weight inside community i
COM = 1:S(1); % Community of node i
for k=1:N
  Neighbor{k} = find(M2(k,:)); % the neighbors of each node i
end

gain = 1; % the index of stoping
while (gain == 1)
  Cost = zeros(1,N); % 
  gain = 0;
  for i=1:N % for each node
    Ci = COM(i); % 
    NB = Neighbor{i}; % the neighbor of node i
    G = zeros(1,N); % Gain vector
    best_increase = -1; % the flag of obtain deltaQ
    Cnew = Ci;
    COM(i) = -1; % remove node i from community
    SumTot(Ci) = SumTot(Ci) - K(i); 
    CNj1 = find(COM==Ci); %
    % sum of weights in community Ci 
